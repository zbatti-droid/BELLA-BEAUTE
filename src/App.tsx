...
import { services, dresses, gallery } from './data/content';
import { calculateCartTotal, formatOrderMessage } from './utils/commerce';
import { AdminPanel, AdminProduct } from './components/AdminPanel';
...
useEffect(()=>{
 ...
 return <div className="site-shell"><div className="scroll-progress" style={{width:`${scrollProgress}%`}} aria-hidden="true" />
  ...
  <div className="hero-image"/><div className="hero-shade"/><div className="hero-content"><p className="kicker">BEAUTY · FASHION · RITUAL</p><h1>{language==='ar'?<>الجمال،<br/><i>بصيغة</i> مختلفة.</>:<>La beauté,<br/><i>autrement.</i></>}</h1><p className="hero-copy">{language==='ar'?'صالون جمال وبوتيك أزياء في أيت ملول، نصمم لك لحظات تشبهك وتبقى في الذاكرة.':'Un salon de beauté et une boutique mode à Ait Melloul, pour des moments qui vous ressemblent.'}</p><div className="hero-actions"><button className="button gold" onClick={()=>setBooking(true)}>{language==='ar'?'احجزي تجربتك':'Réserver votre moment'} <span>↗</span></button><button className="text-button" onClick={()=>scroll('services')}>{language==='ar'?'اكتشفي الخدمات':'Découvrir les services'} <span>↓</span></button></div></div><div className="hero-note">Ait Melloul · Morocco <span>—</span> Since 2018</div><div className="scroll-cue">مرّري للأسفل <span>↓</span></div></section>
   ...
   <section className="services section-dark" id="services"><div className="section-head"><div><div className="section-label">02 / THE RITUALS</div><h2>{language==='ar'?<>طقوس صُمّمت<br/><i>لإيقاعك.</i></>:<>Des rituels<br/><i>pour votre rythme.</i></>}</h2></div><p>{language==='ar'?'خدماتنا ليست قائمة. إنها تجربة متكاملة تبدأ بالاستماع وتنتهي بإطلالة تشبه نسختك الأفضل.':'Nos services sont une expérience complète, de la consultation à la dernière touche.'}</p></div><div className="service-grid">{services.map((s,i)=><motion.article className="service-card" key={s.id} initial={{opacity:0,y:24}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{delay:i*.12}}><div className="service-img"><img src={s.image} alt={s.title} loading="lazy" onError={protectImage}/><span>0{i+1}</span></div><div className="service-meta"><p>{language==='ar'?s.eyebrow:s.eyebrow.replace('Signature','Signature').replace('Bridal','Mariée').replace('Ritual','Rituel')}</p><h3>{language==='ar'?s.title:s.frTitle}</h3><p className="service-description">{language==='ar'?s.text:s.frText}</p><div className="service-bottom"><span>{language==='ar'?s.price:s.price}</span><button onClick={()=>{setBookingService(s.title);setBooking(true)}}>احجزي <span>↗</span></button></div></div></motion.article>)}</div></section>
   <section className="boutique" id="boutique"><div className="section-head"><div><div className="section-label">03 / THE EDIT</div><h2>{language==='ar'?<>قطعٌ لها<br/><i>حضور.</i></>:<>Des pièces<br/><i>qui s’imposent.</i></>}</h2></div><button className="underlink" onClick={()=>wa('مرحباً BELLA BEAUTÉ، أريد معرفة المجموعة المتاحة.')}>اطلبي عبر واتساب <span>↗</span></button></div><div className="category-filters">{[['الكل','Tout'],['فستان سهرة','Soirée'],['زفاف','Mariage'],['إطلالة راقية','Élégant']].map(([item,fr])=><button className={category===item?'active':''} key={item} onClick={()=>setCategory(item)}>{language==='ar'?item:fr}</button>)}</div><div className="dress-grid">{visibleDresses.map((d,i)=><motion.article className="dress-card" key={d.id} initial={{opacity:0,y:24}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{delay:i*.1}}><div className="dress-image" onClick={()=>setSelectedDress(d)}><img src={d.image} alt={d.name} loading="lazy" onError={protectImage}/><button className="heart" aria-label="إضافة للمفضلة">♡</button><span className="dress-index">0{i+1}</span></div><div className="dress-info"><div onClick={()=>setSelectedDress(d)} className="dress-copy"><h3>{language==='ar'?d.name:d.frName}</h3><p>{language==='ar'?d.category:d.frCategory}</p></div><strong>{d.price}</strong></div><button className="add-button" onClick={()=>add(d)}>أضيفي إلى السلة <span>+</span></button></motion.article>)}</div></section>
   <section className="editorial"><div className="editorial-image"/><div className="editorial-copy"><div className="section-label">04 / THE BELLA WOMAN</div><h2>{language==='ar'?<>لأنك لا تبحثين<br/>عن <i>إطلالة.</i></>:<>Vous ne cherchez pas<br/>un <i>look.</i></>}</h2><p>أنتِ تبحثين عن حضور. عن ذلك الشعور حين تدخلين مكاناً وتعرفين أن كل شيء فيكِ في مكانه الصحيح.</p><button className="button light" onClick={()=>{setBookingService('عروس BELLA');setBooking(true)}}>ابدئي قصتك <span>↗</span></button></div></section>
   <section className="gallery-section" id="gallery"><div className="section-label">05 / OUR WORK</div><div className="gallery-title"><h2>{language==='ar'?<>أعمالنا<br/><i>تتحدث.</i></>:<>Notre travail<br/><i>parle.</i></>}</h2><p>{language==='ar'?'تصفيفات، مكياج، عروس وإطلالات صممناها بعناية داخل BELLA BEAUTÉ.':'Coiffures, maquillages, looks de mariée et créations réalisés avec soin chez BELLA BEAUTÉ.'}</p></div><div className="gallery-grid">{(salonImages.length?salonImages:gallery).map((g,i)=><img key={g} className={`g${i+1}`} src={g} alt="BELLA BEAUTÉ" loading="lazy" onError={protectImage}/>)}</div></section>
   <section className="testimonial"><div className="quote-mark">“</div><blockquote key={reviewIndex}>{reviews[reviewIndex].quote}</blockquote><p>— {reviews[reviewIndex].name}</p><div className="review-controls"><button onClick={()=>setReviewIndex((reviewIndex-1+reviews.length)%reviews.length)} aria-label="التقييم السابق">←</button><span>0{reviewIndex+1} / 0{reviews.length}</span><button onClick={()=>setReviewIndex((reviewIndex+1)%reviews.length)} aria-label="التقييم التالي">→</button></div></section>
   <section className="contact section-dark" id="contact"><div className="contact-top"><div><div className="section-label">06 / COME BY</div><h2>{language==='ar'?<>نلتقي<br/><i>قريباً.</i></>:<>À très<br/><i>bientôt.</i></>}</h2></div><p>{language==='ar'?'للحجز أو الاستفسار، اكتبي لنا. سنكون سعداء بمساعدتك في اختيار التجربة المناسبة.':'Pour réserver ou poser une question, écrivez-nous. Nous vous aiderons à choisir votre expérience.'}</p></div><div className="contact-grid"><div><span>العنوان</span><p>Ait Melloul, Agadir<br/>Morocco</p><button className="underlink" onClick={()=>window.open('https://maps.google.com/?q=Ait+Melloul+Morocco','_blank')}>افتحي الخريطة ↗</button></div><div><span>ساعات العمل</span><p><em className={isOpen?'open-status':'closed-status'}>{isOpen?(language==='ar'?'● مفتوح الآن':'● Ouvert maintenant'):(language==='ar'?'● مغلق حالياً':'● Fermé actuellement')}</em><br/>الإثنين — الأحد<br/>09:00 — 00:00</p></div><div><span>تواصلي</span><p><a href="tel:+212631751974">+212 631 751 974</a><br/><button className="wa-link" onClick={()=>wa('مرحباً BELLA BEAUTÉ، أريد حجز موعد.')}>WhatsApp ↗</button><br/><a className="instagram-link" href="https://www.instagram.com/" target="_blank" rel="noreferrer">Instagram ↗</a></p></div></div></section>
  </main><footer><div className="brand">BELLA <span>BEAUTÉ</span></div><p>© 2026 BELLA BEAUTÉ. Made with intention.</p><p>Ait Melloul · Morocco</p><a className="footer-social" href="https://www.instagram.com/" target="_blank" rel="noreferrer">Instagram ↗</a></footer>
